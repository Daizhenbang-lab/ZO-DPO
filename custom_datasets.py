...
import tqdm
import random
# from bs4 import BeautifulSoup, NavigableString  # uncomment if you want to use the SE dataset
import numpy as np
...
class PrefDataset(Dataset):
    """
        Generic dataset for preference optimization.
        Takes in the name of the dataset.
    """
    def __init__(self, name, split, tokenizer, max_length=None) -> None:
        self.tokenizer = tokenizer
        self.max_length = max_length if max_length is not None else float('inf')
        self.flat_data = [(data["prompt"], data['responses'], data['pairs'][0]) for data in get_dataset(name, split)]

    def __len__(self):
        return len(self.flat_data)

    def __getitem__(self, idx):
        # prompt is user query, responses is a dict/list with positive/negative response
        prompt, responses, pairs = self.flat_data[idx]
        ...
        answers = deepcopy(prompt)
        # append positive response
        answers.append({"role": "assistant", "content": responses[pairs[0]]})
        # tokenize positive response
        pos = self.tokenizer.apply_chat_template(
            answers,
            return_tensors='pt',
        )
        pos = torch.cat((pos, torch.tensor([[self.tokenizer.eos_token_id]])), axis=1)

        # remove positive response
        answers.pop()
        # append negative response
        answers.append({"role": "assistant", "content": responses[pairs[1]]})
        # tokenize negative response
        neg = self.tokenizer.apply_chat_template(
            answers,
            return_tensors='pt',
        )
        neg = torch.cat((neg, torch.tensor([[self.tokenizer.eos_token_id]])), axis=1)

        # we mask the prompt tokens, -100 is ignored by the cross-entropy loss
        pos_labels = torch.clone(pos)
        pos_labels[:, : prompt_len] = -100
        neg_labels = torch.clone(neg)
        neg_labels[:, : prompt_len] = -100
        return_dict = {
            "pos_input_ids": pos.flatten(),
            "neg_input_ids": neg.flatten(),
            "pos_labels": pos_labels.flatten(),
            "neg_labels": neg_labels.flatten()
        }
        return_dict = {k: v[:self.max_length] if len(v) > self.max_length else v for k, v in return_dict.items()}
        return return_dict

class ClassificationDataset(PrefDataset):
    ...
    def __getitem__(self, idx):
        ...
        original_data = super().__getitem__(original_idx)

        if original_idx < len(self.flat_data):
            input_ids = original_data['pos_input_ids']
            labels = torch.tensor(1)  # the label of positive is 1
        else:
            input_ids = original_data['neg_input_ids']
            labels = torch.tensor(0)  # the label of negative is 0


        input_ids = input_ids[:self.max_length] if len(input_ids) > self.max_length else input_ids
...
# Everything below is from https://github.com/eric-mitchell/direct-preference-optimization
# with minor modifications to fit chat template and remove unnecessary code
def extract_anthropic_prompt(prompt_and_response):
    """Extract the anthropic prompt from a prompt and response pair."""
    # Regex pattern to match "\n\n Human:" and "\n\n Assistant:" with their content, allowing for optional spaces
    pattern = r'\n\n\s*(Human|Assistant): (.*?)(?=\n\n\s*Human: |\n\n\s*Assistant: |\Z)'

    # Find all matches
    matches = re.findall(pattern, prompt_and_response, re.DOTALL)

    # Convert matches to list of dicts
    conversations = []
    last_role = ""
    for role, content in matches:
        current_role = "user" if "human" in role.lower() else "assistant"
        if last_role == current_role:
            content += conversations.pop()["content"]
        last_role = current_role
        conversations.append({"role": current_role, "content": content.strip()})

    # Remove the last Assistant: response if present
    last_assistant_position = len(prompt_and_response)
    if conversations and conversations[-1]['role'] == 'assistant':
        last_assistant_content = conversations.pop()['content']
        last_assistant_position -= len(last_assistant_content)
    else:
        return None, None

    if conversations[-1]["role"] != "user":
        return
    length_up_to_last_assistant = last_assistant_position

    return conversations, length_up_to_last_assistant
...
def split_prompt_and_responses(ex):
    prompt, prompt_len = extract_anthropic_prompt(ex['chosen'])
    if prompt is None:
        return None, None, None
    chosen_response = ex['chosen'][prompt_len:]
    rejected_response = ex['rejected'][prompt_len:]
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None):
    ...
    if name != "hh":
        raise NotImplementedError("Only hh dataset supported for now. Other datasets need to be changed for the new format that supports chat template.")
    if name == 'shp':
